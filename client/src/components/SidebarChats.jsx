...
// switchRoom is now passed from Chats.js — it handles leaving the old room
// before joining the new one, preventing socket message bleed.
const SidebarChats = ({
  ...
  const fetchLastMessages = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.status === 200) {
        // Stringify keys: DB may return numeric ids; selectedUser.id could be a
        // number. Using String() keys prevents the "1" !== 1 mismatch.
        const msgs = response.data.data.reduce((acc, msg) => {
          acc[String(msg.other_user_id)] = {
            ...msg,
            unread_count: Number(msg.unread_count) || 0,
  ...
  const roomSet = async (selectedUser) => {
    ...
    try {
      ...
      if (response.data.status === 200) {
        setRoomId(response.data.data.room_id);
        switchRoom(response.data.data.room_name); // leave old room, join new one
        setSelectedUser(selectedUser.username);
        setReceiverId(selectedUser.id);
        setIsGroup(false);
      } else {
        console.error('Error creating/fetching room:', response.data.msg);
      }
    } catch (error) {
