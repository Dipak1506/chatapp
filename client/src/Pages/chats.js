...
const Chats = (props) => {
  ...
  useEffect(() => {
    ...
    if (isgroup) {
      if (!roomId) return;
      apiConfig.get('/getmessages', { params: { isgroup: true, roomId } })
        .then(res => {
          if (res.data) setMessages(res.data.data);
      ...
      if (!receiverId) return;
      apiConfig.get('/getmessages', {
        params: { userId: props.userId, selectedUserId: receiverId },
      })
        .then(res => { if (res.data) setMessages(res.data.data); })
        .catch(err => console.error('Error fetching DM messages:', err));
    }
  }, [receiverId, roomId, isgroup, props.userId]);
  ...
  useEffect(() => {
    apiConfig.get('/getusers')
      .then(response => {
        if (response.data && Array.isArray(response.data.data)) {
          setUserData(response.data.data.filter(u => u.id !== props.userId));
        } else {
          setUserData([]);
        }
      })
  ...
  // ── switchRoom: leave previous, join next, update local room state ─────────
  // Every conversation-switch path goes through here so leave/join is always
  // paired and the local room state is always up to date before any child
  // re-renders.
  const switchRoom = (newRoom) => {
    ...
    activeRoomRef.current = newRoom;
    setRoom(newRoom);               // synchronous local update
    socket.emit('joinRoom', newRoom);
  };

  // ── Group chat creation ────────────────────────────────────────────────────
  const handleGroupChatCreation = async (selectedUsers) => {
    const roomName = selectedUsers.map(u => u.username).sort().join('-');
    switchRoom(roomName);           // sets local room immediately
    setReceiverId(selectedUsers.map(u => u.id));
    setIsGroup(true);

    try {
      const response = await apiConfig.get('/createroom', {
        params: { room_name: roomName, isgroup: true },
      });
      if (response.data.status === 200) {
        const newRoomId = response.data.data.room_id;
        setRoomId(newRoomId);
        setIsGroupChat(true);
        apiConfig.get('/createGroups', {
          params: { room_id: newRoomId, members: selectedUsers, group_name: roomName },
        });
      } else {
        console.error('Error creating group room:', response.data.msg);
      }
    } catch (err) {
  ...
  const handleGroupNameUpdate = (newGroupName) => {
    setGroups(prev =>
      prev.map(g => g.room_id === roomId ? { ...g, group_name: newGroupName } : g)
    );
    setSelectedUser(newGroupName);
  };
