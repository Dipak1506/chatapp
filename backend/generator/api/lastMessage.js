...
//     console.log("last msg usr id :", userId);
...
//      WHERE m2.sender_id <> $1 AND m2.receiver_id = $1 AND m2.is_read = FALSE) AS unread_count
// FROM messages m1
...
//     //         is_read,
//     //         (SELECT COUNT(*) FROM messages m2 
//     //          WHERE m2.sender_id = CASE WHEN sender_id = $1 THEN receiver_id ELSE sender_id END
...
`;
    ...
    connection.query(selectQuery, [userId, senderId], (err, data) => {
        if (err) {
            console.log(err);
            res.send({ status: 201, message: "Error getting last messages" });
        } else {
